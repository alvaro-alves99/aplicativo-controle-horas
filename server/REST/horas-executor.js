module.exports = function(app){
	...
	return app.get('/horas-executor', function(req, res){
		...
		connection.query('SELECT horaEntrada, horaSaida FROM tarefas WHERE executor="' + req.query.executor + '" AND horaSaida IS NOT NULL;', function(err, results){
			if(err){
		...
		function calculaData(results){
			...
			var resultados = results.map(function(task){
				...
				return {"hora": hora,"minutos": restoMinutos};
			});
